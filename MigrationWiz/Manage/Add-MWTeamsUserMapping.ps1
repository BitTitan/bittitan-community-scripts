...
.SYNOPSIS
    Script to add UserMapping Advanced Options to MigrationWiz Teams Projects.

.DESCRIPTION
    This script will add UserMapping Advanced Options entries to a MigrationWiz Teams Project.
...
{
    Write-Error "Invalid csv format, please ensure you have two columns with the header Source, Destination."
    Exit
}
...
$advancedOptions = $csvContent | ForEach-Object {"UserMapping=""$($_.SourceAddress)->$($_.DestinationAddress)"""}
...
if ($mailboxConnector.ProjectType -ne "Teamwork")
{
    Write-Error "Specified ProjectId does not correspond to a MigrationWiz Teams Project."
    Exit
}
