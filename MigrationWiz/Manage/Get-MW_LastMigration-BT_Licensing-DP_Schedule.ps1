...
.SYNOPSIS
    This script exports a set of important information about users in a specific project.
     
.DESCRIPTION
    This script will export the migration configuration and/or last migration submission info and/or Licensing info 
    and/or DMA/DeploymentPro configuration/status and/or Office 365 Multi-Factor authentication info for the migrations 
    under the selected project or for all projects.
...
Write-Host $msg
    ...
    Write-Host
    do {
        $confirm = (Read-Host -prompt "Do you want the script to automatically open the CSV file generated?  [Y]es or [N]o")
        if($confirm.ToLower() -eq "y") {
            $openCSVFile = $true
        }
        else {
